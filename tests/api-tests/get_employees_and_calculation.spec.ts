...
test.describe("Get employee and verify calculation", () => {
  let userApi: UserApi;
  let getEmployeeId: string;

  test.afterEach(async ({ request }) => {
    // Cleanup: Delete employee after test (even if test fails)
    if (getEmployeeId) {
      const userApi = new UserApi(request);
      const deleteResponse = await userApi.deleteEmployee(getEmployeeId);
      console.log(
        `Cleanup: Deleted employee ${getEmployeeId}, status: ${deleteResponse.status()}`,
      );
    }
  });
  ...
  test("Create employee, verify data and correct calculation with GET, delete employee via API", async ({
    ...
    await test.step("Create new employee via API", async () => {
      ...
      const createdEmployeeData =
        await userApi.getEmployeeDataFromResponse(createResponse);
    ...
    await test.step("Get created employee and verify data calculation", async () => {
      ...
      const retrievedEmployeeData =
        await userApi.getEmployeeDataFromResponse(getEmployeeResponse);

      //Verify data about employee and calculation
      expect(retrievedEmployeeData.id, "Employee ID matches").toBe(
        getEmployeeId,
      );
      ...
      expect(retrievedEmployeeData.lastName, "Last name matches").toBe(
        lastName,
      );
      ...
      expect(retrievedEmployeeData.net, "Net is 1826.92").toBeCloseTo(
        1826.92,
        2,
      );
