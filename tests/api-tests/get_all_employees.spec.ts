...
test.describe("Get all employees with create/delete flow", () => {
  ...
  test("Create new employee, returns total number of employees, delete created employee via API", async ({
    ...
    await test.step("Delete created employee", async () => {
      ...
      const getAllEmployeesResponseAfterDelete =
        await userApi.getAllEmployees();
      expect(
      ...
      getEmployeeId = ""; // Mark as deleted to skip afterEach cleanup
    });
  });
});
